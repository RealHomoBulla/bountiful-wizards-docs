...
⚖️ Репозиторий **публичный** — твоё решение 25.08 после того, как я назвал, что именно выставляется
наружу (ссылка на приватный vault, два пути с виндовым именем, весь дизайн пака). Зато читается
откуда угодно, включая `raw.githubusercontent.com`, без всяких коннекторов.
🔑 **Что это значит на практике: всё, что попадёт в корень `claudework/`, станет публичным при
следующем синке.** Секрет, ключ или чужие персональные данные туда класть нельзя — не потому что
правило, а потому что синк их опубликует молча.
